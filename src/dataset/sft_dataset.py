...
class SupervisedValidationDataset(Dataset):
    """Dataset for supervised fine-tuning."""
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources:
            ...
            image_files = sources["image"]
            validation_image_folder = self.data_args.validation_image_folder
            ...
            for image_file in image_files:
                if not os.path.exists(image_file):
                    if not image_file.startswith("http"):
                        image_file = os.path.join(validation_image_folder, image_file)
                images.append(get_image_info(image_file, self.image_min_pixel, self.image_max_pixel, self.image_resized_w, self.image_resized_h))
            ...
            video_files = sources["video"]
            video_folder = self.data_args.validation_image_folder
...
def make_supervised_data_module(model_id, processor, data_args):
    ...
    validation_dataset = SupervisedValidationDataset(
